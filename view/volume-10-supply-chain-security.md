...
14. [零信任软件供应链原则](../struct/10-supply-chain-security/05-zero-trust-supply-chain/zero-trust-principles.md)
15. [零信任软件供应链架构设计模板](../struct/10-supply-chain-security/05-zero-trust-supply-chain/zero-trust-template.md)
16. [EU Cyber Resilience Act (CRA) 2024/2847 合规检查清单](../struct/10-supply-chain-security/06-case-studies/eu-cra-checklist.md)
17. [欧盟网络弹性法案 (EU CRA) 合规指南](../struct/10-supply-chain-security/06-case-studies/eu-cra-compliance.md)
18. [供应链安全 — 国际化案例集](../struct/10-supply-chain-security/06-case-studies/international-cases.md)
...
26. [SLSA Build Level 4 概念验证（PoC）](../struct/10-supply-chain-security/13-slsa-l4-poc/README.md)
...
该 provenance 符合 SLSA 1.0 Build L2；若配合可复用工作流（reusable workflow）并限制工作流权限，可达到 Build L3。下游消费者使用 `gh attestation verify` 即可验证。
...
<!-- SOURCE: struct/10-supply-chain-security/06-case-studies/international-cases.md -->
...
## 3. Rev.1 草案相对现行正式版（SSDF v1.1）的关键变化

NIST 于 2025-12-17 发布 SP 800-218 Rev.1 的 **Initial Public Draft（征求意见稿，SSDF v1.2）**，最终版预计 2026-Q3。
与 SSDF v1.1（SP 800-218 Final，现行正式版）及早期草案相比，Rev.1（IPD）的主要演进包括：
...
5. **示例更新**：Rev.1 草案补充了云原生、容器镜像与 SBOM 生成的示例，直接回应了软件复用框架中容器化交付的场景需求。
...
以下清单将 SSDF 1.2 的关键实践转化为 10 项可落地的检查项，并映射到 SLSA（Supply-chain Levels for Software Artifacts）等级要求。
...
- NIST. *Secure Software Development Framework (SSDF) Version 1.2*. NIST SP 800-218 Rev.1 (Initial Public Draft), 2025.
...
对于来自开源社区的组件，优先选择已采用 Sigstore 签名、且在 SLSA（Supply-chain Levels for Software Artifacts）框架下达到 L2 及以上构建等级的项目。
...
- [C-05 GUAC 供应链图谱与复用风险评估](#c-05-guac-供应链图谱与复用风险评估)
  ...
  - [5. 实践场景：复用全生命周期](#5-实践场景复用全生命周期)
    ...
    - [5.5 反例 / 反模式](#55-反例--反模式)
      - [反例 A：SolarWinds 响应中的可见性缺失](#反例-asolarwinds-响应中的可见性缺失)
      - [反例 B：SBOM 散落导致 GUAC 无法构建完整图谱](#反例-bsbom-散落导致-guac-无法构建完整图谱)
...
它不是一个替代 SCA 或 SLSA（Supply-chain Levels for Software Artifacts）的工具，而是一个**位于它们之上的聚合分析层**，将各工具的输出连接成一张可推理、可查询的安全知识图谱。
...
<!-- SOURCE: struct/10-supply-chain-security/13-slsa-l4-poc/README.md -->
...
![10 供应链安全 — 安全控制采纳决策树](../struct/99-reference/visualizations/decision-trees/10-supply-chain-security-decision-tree.svg)
...
2024 年起 GitHub Actions 原生支持 `actions/attest-build-provenance`，为任意构建产物生成符合 SLSA 1.0 Build L2 的证明；结合可复用工作流可达 Build L3。npm Trusted Publishing 与 PyPI attestations（PEP 740）已使主流生态默认携带 provenance，显著降低开源消费者验证成本。
