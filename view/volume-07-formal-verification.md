...
1. [T08: A2A Task 状态机的 TLA+ 规约说明](07-formal-verification/01-tla-plus/a2a-task-lifecycle.md)
2. [TLA+ 案例库总览](07-formal-verification/01-tla-plus/case-library.md)
3. [T07: MCP Server 能力协商协议的 TLA+ 规约说明](07-formal-verification/01-tla-plus/mcp-capability-negotiation.md)
4. [T06: 分布式支付服务组件的 TLA+ 规约说明](07-formal-verification/01-tla-plus/payment-service.md)
5. [T11: 组件依赖无环性验证 (Alloy)](07-formal-verification/02-alloy/component-dependency.md)
6. [T13: 跨层复用映射的约束验证 (Alloy)](07-formal-verification/02-alloy/cross-layer-mapping.md)
7. [T14: ISA-95 资源层次一致性验证 (Alloy)](07-formal-verification/02-alloy/isa95-hierarchy.md)
8. [T12: MCP Tool 能力依赖图验证 (Alloy)](07-formal-verification/02-alloy/mcp-tool-graph.md)
9. [Coq/Rocq & Isabelle/HOL 可复用组件验证案例](07-formal-verification/03-coq-isabelle/README.md)
10. [Coq / Isabelle Theorem Proving for Safety-Critical Components](07-formal-verification/03-coq-isabelle/security-critical-components.md)
11. [Coq/Isabelle 安全关键组件定理证明纲要](07-formal-verification/03-coq-isabelle/theorem-proving-guidelines.md)
12. [Cargo 依赖解析的 SAT 求解：PubGrub 算法与复用一致性](07-formal-verification/04-rust-type-system/cargo-sat-resolution.md)
13. [Rust 类型系统：编译期复用安全的形式化语义](07-formal-verification/04-rust-type-system/formal-semantics.md)
14. [Rust Polonius 借用检查器 vs NLL：形式化对比与复用影响](07-formal-verification/04-rust-type-system/polonius-vs-nll.md)
15. [Rust 形式化验证工具链实践（2025‑2026）](07-formal-verification/04-rust-type-system/toolchain-practice.md)
16. [Rust unsafe 边界的验证策略：工具链对比与可复用组件检查清单](07-formal-verification/04-rust-type-system/unsafe-verification.md)
17. [SPARK Ada 飞控软件契约验证案例](07-formal-verification/05-spark-ada/flight-control-contracts.md)
18. [DO-178C MC/DC 的形式化定义与 SPARK 验证](07-formal-verification/05-spark-ada/mcdc-formalization.md)
19. [SPARK Ada 形式化验证与工业复用](07-formal-verification/05-spark-ada/spark-ada-do333-industrial.md)
20. [Event-B 与 B Method：铁路信号系统的精化复用](07-formal-verification/06-b-method/event-b-railway-refinement.md)
21. [B Method / Event-B 铁路信号系统精化链案例](07-formal-verification/06-b-method/railway-signaling-refinement.md)
22. [IEEE 1012-2024 验证与确认标准在复用工程中的应用](07-formal-verification/07-vv-standards/ieee-1012-2024-reuse-vv.md)
23. [形式化验证前沿：LLM + 定理证明与 Agent 行为合约](07-formal-verification/08-emerging-trends/llm-formal-verification-frontier.md)
24. [SPARK Ada vs Rust：安全关键系统验证方法对比矩阵](07-formal-verification/09-comparative-matrices/spark-ada-vs-rust-verification-matrix.md)
25. [07 形式化验证 — 内容梳理报告](07-formal-verification/content-audit-2026-06-08.md)
26. [07 形式化验证与复用正确性](07-formal-verification/README.md)
...
Agent-to-Agent（A2A）协议是由 Google 于 2025 年 4 月发起、现由 Linux Foundation 维护的开放标准，旨在实现 AI Agent 之间的互操作通信[^7]。与 MCP（Model Context Protocol）解决"Agent ↔ Tool"的垂直集成问题不同，A2A 聚焦于"Agent ↔ Agent"的水平协作问题。A2A v1.0 规范（2026 年 3 月正式发布）定义了一个精细的任务生命周期（Task Lifecycle）状态机，这是协议的核心语义骨架[^8]。
...
值得注意的是，A2A v1.0 还定义了 `auth_required` 和 `rejected` 状态，本规约出于简化考虑聚焦于最核心六态模型，但其扩展至八态模型在结构上完全同质。
...
### 3.1 TerminalNoMessages（终止状态无消息增长）
...
### 3.2 CompletedHasArtifact（completed 必须有 Artifact）
...
### 3.3 NonTerminalHasMessages（非终止状态必有消息历史）
...
### 3.4 FailedCanceledNoArtifacts（失败和取消状态 Artifact 为空）
...
### 4.1 WorkingEventuallyTerminates（working 最终终止）
...
### 4.2 InputRequiredEventuallyResolved（input_required 最终解决）
...
## 5. 与 A2A v1.0 规范的对应关系

本规约与 A2A v1.0 规范的对应如下[^8]：
...
[^3]: AdaCore. *GNATcoverage User's Guide — DO-178C / ED-12C Qualifiable Coverage Analysis*. [https://www.adacore.com/gnatcoverage](https://www.adacore.com/gnatcoverage). 详细说明 GNATcoverage 的 MC/DC 提取、分析和报告机制。
