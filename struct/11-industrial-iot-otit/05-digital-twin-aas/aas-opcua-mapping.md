...
AAS (Asset Administration Shell) 是工业 4.0 的数字孪生核心标准，IEC 63278-1:2023 定义了其元模型。
OPC UA (IEC 62541) 是工业通信的事实标准。二者的映射实现了三层价值：
...
同一类型的设备共享相同的子模型模板。
例如所有电机都使用 `Nameplate` + `Technical Data` 子模型。
OPC UA 中对应为**类型定义复用**：
...
设备厂商根据 I4AAS Companion Specification 实现 OPC UA Server，自动暴露 AAS 结构。
认证流程：
...
AAS 配置以 AASX（ZIP + JSON/XML + 附件）格式交换，实现跨工具、跨厂商的复用。
AASX 包中的 OPC UA NodeSet 文件可直接导入 UA Server：
