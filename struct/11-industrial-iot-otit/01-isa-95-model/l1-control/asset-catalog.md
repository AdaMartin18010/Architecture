...
L1 控制层是 ISA-95 中直接执行物理过程控制的层级，核心资产为 **PLC/DCS 程序单元**。本层复用的关键在于：将经过现场验证的算法封装为可移植的 **功能块 (Function Block, FB)**、**用户数据类型 (UDT)** 与控制策略模板，实现"一次开发、多机型部署"。PLCopen 运动控制库的成功实践表明，标准化 FB 可将跨厂商移植成本降低 40% 以上。
