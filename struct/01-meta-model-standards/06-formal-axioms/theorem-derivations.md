...
>
> **补强方向**：(a) 增加定义“同一架构复用 ≡ 存在非空共享约束子集”，使第 4 步成为定义展开；或 (b) 将结论弱化为“存在（可能为空的）最大共享约束子集”并明确 ⊨ 的语义。详见 [`formalization-mapping.md`](formalization-mapping.md) §5.1。
...
>
> **补强方向**：增补前提（完备度量空间 + 反馈是资产状态的函数 + 𝒢̃ 为 L<1 的压缩映射），结论限定为“迭代收敛到唯一不动点”，删去“最优”表述。详见 [`formalization-mapping.md`](formalization-mapping.md) §5.2。
