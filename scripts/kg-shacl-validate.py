...
约束清单：
  ...
  C6 EVOLVED-TYPE(error)   EVOLVED_FROM 两端实体类型必须 ∈ {Standard, Protocol}
                           （本体 domain/range 已放宽为 Entity，类型约束在本检查中落实）
...
DECLARED_ENDPOINTS = {
    ...
    # 本体 domain/range 已放宽为 Entity→Entity（覆盖 Protocol 版本线），
    # 端点类型约束由 C6 EVOLVED-TYPE 落实（仅 Standard/Protocol 合法）。
    "evolvedFrom": ("Entity", {"Entity"}),
}
JSONL_REL_TO_PROP = {
...
# ---------------------------------------------------------------------------
# C6: EVOLVED_FROM 端点类型约束（本体已放宽为 Entity→Entity，类型在此落实）
# ---------------------------------------------------------------------------

EVOLVED_FROM_TYPES = {"Standard", "Protocol"}


def check_evolved_type(entities: Dict[str, dict], relations: List[dict]):
    """EVOLVED_FROM 两端实体类型必须 ∈ {Standard, Protocol}。"""
    violations = []
    for r in relations:
        if r["relation"] != "EVOLVED_FROM":
            continue
        s, t = entities.get(r["source_id"]), entities.get(r["target_id"])
        if not s or not t:
            continue  # dangling 由 C2 报告
        st, tt = s.get("type"), t.get("type")
        if st not in EVOLVED_FROM_TYPES or tt not in EVOLVED_FROM_TYPES:
            violations.append((r, st, tt))
    return violations
...
def main() -> int:
    ...
    evolved_type_viol = check_evolved_type(entities, relations)
    ...
    errors = {
        ...
        "C6 EVOLVED-TYPE": len(evolved_type_viol),
    }
    ...
    # C6
    lines += ["## C6 EVOLVED-TYPE — EVOLVED_FROM 端点类型约束", ""]
    lines.append("规则：本体 `evolvedFrom` domain/range 已放宽为 Entity→Entity（覆盖 Protocol 版本线），"
                 "端点类型须 ∈ {Standard, Protocol}，由本检查落实。")
    lines.append("")
    if not evolved_type_viol:
        lines.append("- ✅ 无违例。")
    else:
        lines += fmt_samples(
            evolved_type_viol, n,
            lambda x: f"- `{x[0]['source_id']}` ({x[1]}) -EVOLVED_FROM-> `{x[0]['target_id']}` ({x[2]})",
        )
    lines.append("")
    ...
    lines.append("")
    lines.append("### 已知语义限制")
    lines.append("- `evolvedFrom` 本体 domain/range 已放宽为 Entity→Entity（2026-07-12），端点类型由 C6 "
                 "EVOLVED-TYPE 约束为 Standard/Protocol；MCP 2024-11-05→2025-11-25、A2A v0.3.0→v1.0.1 "
                 "谱系边已由 kg-relation-enricher.py 补全。A2A v1.1 / A2A 2.0 为预测版本"
                 "（见 protocol-analysis.md 2026-Q4/2027-H1 路线图），谱系边待正式发布后补。")
    lines.append("")
